...
export class GalleryComponent implements OnInit, OnDestroy, OnChanges {
    ...
    public ngOnChanges(changes: SimpleChanges) {
        ...
        if (changes['providedGalleryName'] != null) this.fetchDataAndRender();
        else this.render();
    }
    ...
    private fetchDataAndRender() {
        ...
        this.http.get<any[]>(this.imageDataCompletePath).subscribe(
            data => {
                this.images = data;
                this.ImageService.updateImages(this.images);
            ...
            err =>
                this.providedMetadataUri
                    ? console.error(
                          "Provided endpoint '" +
                              this.providedMetadataUri +
                              "' did not serve metadata correctly or in the expected format. \n\nSee here for more information: https://github.com/BenjaminBrandmeier/angular2-image-gallery/blob/master/docs/externalDataSource.md,\n\nOriginal error: " +
                              err
                      )
                    : console.error(
                          'Did you run the convert script from angular2-image-gallery for your images first? Original error: ' +
                              err
                      ),
            () => undefined
        );
    ...
    private calcRowHeight(imgRow: any[]) {
        ...
        let ratio =
            ...
            originalRowWidth;
        let rowHeight =
            imgRow[0][this.minimalQualityCategory]['height'] * ratio;
    ...
    private calcOriginalRowWidth(imgRow: any[]) {
        ...
        imgRow.forEach(img => {
            let individualRatio =
                this.calcIdealHeight() /
                img[this.minimalQualityCategory]['height'];
        ...
        this.gallery.forEach(imgRow => {
            ...
            if (imgRow !== this.gallery[this.gallery.length - 1]) {
                ...
                imgRow.forEach((img: any) => {
                    img['width'] =
                        img[this.minimalQualityCategory]['width'] * ratio;
                    img['height'] =
                        img[this.minimalQualityCategory]['height'] * ratio;
        ...
        if (
            ...
            (imageElements.length > 0 &&
                this.isScrolledIntoView(
                    imageElements[imageCounter].nativeElement
                ))
        ) {
